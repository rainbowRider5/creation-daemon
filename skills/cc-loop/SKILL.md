...
4. **Dispatch by state.** Look at the ticket's current state:
   ...
   - `refined` → the autonomous loop auto-approves when safe: if the refinement artifact has no `## Open Questions` section (or that section is empty), call `cc_transition_state` with `new_state: ready`. Otherwise, call `cc_block_ticket` with the unanswered questions so a human can answer. (A human running `/cc-refine` can still manually block instead of advancing.)
   ...
   - `in-review` → follow the `/cc-review` process. If the PR is already merged, `/cc-review` will transition the ticket to `done`.
