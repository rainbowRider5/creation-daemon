---
name: cd-review
description: 'Review the PR for a cd:in-review ticket, write a review artifact, then approve or request changes (→ cd:in-progress). Re-run after merge to finalize to cd:done.'
allowed-tools:
...
6. **Write the review artifact.** Call `cd_write_artifact` with:
...
7. **Decide the next action.**
   - **Approved:**
     ...
     - Re-check merge state: `gh pr view <pr-number> --json state -q .state`.
       - If `MERGED` (e.g. auto-merge was on), call `cd_transition_state` with `new_state: done`.
       - If `OPEN`, leave the ticket in `cd:in-review` and tell the user: "PR approved. After merging, run `/cd-review $ARGUMENTS` again to finalize the ticket to `cd:done`." The next `/cd-review` invocation will pick up the merged state via step 3 and transition automatically.
