...
**Goal:** Restructure creation-daemon from a v2 monolithic CLI to a v3 MCP server that exposes ticket lifecycle tools (create, transition, schedule, block/unblock, artifacts, status).
...
export function registerTools(server: McpServer) {
      ...
      if (!currentState) {
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error: Issue #${issue_number} has no cd:* state label.`,
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    async () => {
      ...
      const blockedTickets = issues.filter((issue) =>
        issue.labels.includes('cd:blocked'),
      );
  ...
  server.tool(
    'cd_ensure_labels',
    'Create cd:* labels on the repo if missing',
    {},
    async () => {
...
Expected: Response listing all 11 tools (cd_create_ticket, cd_list_tickets, cd_get_ticket, cd_transition_state, cd_pick_next_ticket, cd_write_artifact, cd_read_artifacts, cd_block_ticket, cd_unblock_check, cd_get_status, cd_ensure_labels).
