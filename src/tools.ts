...
export function registerTools(server: McpServer) {
  server.registerTool(
    'cc_create_ticket',
    {
      description: 'Create a GitHub issue with cc:draft label',
      inputSchema: {
  ...
  server.registerTool(
    'cc_list_tickets',
    {
      description: 'List all cc:* issues with their state and priority',
      inputSchema: {
      ...
      if (!currentState) {
        return {
          content: [
            {
              type: 'text' as const,
              text: `Error: Issue #${String(issue_number)} has no cc:* state label.`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    ...
    async () => {
      const issues = await listIssuesByLabel('cc:');
      const blockedTickets = issues.filter((issue) => issue.labels.includes('cc:blocked'));
  ...
  server.registerTool(
    'cc_ensure_labels',
    {
      description: 'Create cc:* labels on the repo if missing',
      inputSchema: {},
    },
    async () => {
