import { z } from 'zod';
import { validateTransition, getStateFromLabels, labelForState, parsePriority, } from './tickets/state-machine.js';
...
import { createIssue, getIssue, listIssuesByLabel, updateLabels, ensureLabels, postComment, getCommentsSince, } from './github.js';
...
export function registerTools(server) {
    server.registerTool('cd_create_ticket', {
    ...
    server.registerTool('cd_list_tickets', {
    ...
    server.registerTool('cd_get_ticket', {
    ...
    server.registerTool('cd_transition_state', {
    ...
    server.registerTool('cd_pick_next_ticket', {
        ...
        if (sorted.length === 0) {
            return {
                content: [{ type: 'text', text: 'No actionable tickets found.' }],
            };
        }
        const next = sorted[0];
        return {
    ...
    server.registerTool('cd_write_artifact', {
        ...
        return {
            content: [{ type: 'text', text: `Artifact written: ${path}` }],
        };
    });
    server.registerTool('cd_read_artifacts', {
        ...
        if (artifacts.length === 0) {
            return Promise.resolve({
                content: [
                    { type: 'text', text: `No artifacts found for #${String(issue_number)}.` },
                ],
            });
        }
    ...
    server.registerTool('cd_block_ticket', {
    ...
    server.registerTool('cd_unblock_check', {
        ...
        for (const issue of blockedTickets) {
            ...
            if (comments.length > 0) {
                ...
                unblocked.push(`#${String(issue.number)} → ${restoredState} (reply from ${comments[0].user})`);
            }
        }
        const text = unblocked.length > 0
    ...
    server.registerTool('cd_get_status', {
    ...
    server.registerTool('cd_ensure_labels', {
