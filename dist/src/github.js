...
export function getRepoContext() {
    ...
    if (!parsed)
        throw new Error(`Not a GitHub remote: ${url}`);
    return parsed;
}
...
export async function updateLabels(issueNumber, add, remove) {
    const { owner, repo } = getRepoContext();
    const octokit = getOctokit();
...
export async function getCommentsSince(issueNumber, since) {
    const { owner, repo } = getRepoContext();
    const octokit = getOctokit();
